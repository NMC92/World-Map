document.body.style.zoom="1%";$(function()
{$('#span').hide();function orderPins(pins)
{let auxPins=[];for(let i=0;i<pins.length;i++)
auxPins.push(pins[i]);auxPins.sort(function(a,b)
...
{let min=0,max=array.length-1,guess,guessValue;while(min<=max)
{guess=Math.floor((max+min)/2);guessValue=array[guess];if(guessValue===targetValue)
return array[guess];else if(guessValue<targetValue)
min=guess+1;else
max=guess-1;}
return-1;}
function updateProgressBar(cur,tot)
{let value=Math.floor(cur*100/tot);if(value>100)
...
{let parent=targetElement.parentNode;parent.appendChild(targetElement);}
function applyEvList(globEl,element)
{function loadPins()
{let pinsPath=parseInt(globEl.Element.AdminLevel_Child)>1?`${txt1}${countryCode}${txt8}${txt4}`:`${txt1}${txt8}${txt4}`;$.getJSON(pinsPath,function($pinsFile)
{if($pinsFile.length>0)
$pinsFile.forEach((pin)=>{if(!loadedPins.includes(pin)&&parseInt(pin.Element_Id)!==-1)
loadedPins.push(pin);});}).fail(function()
...
$('#back').show();switch(parseInt(globEl.Element.AdminLevel_Child))
{case 0:case 1:case 2:case 3:case 4:case 5:case 6:case 7:case 8:case 9:case 10:case 11:case 12:global.filter(el2=>parseInt(el2.Element.AdminLevel_Child)>parseInt(globEl.Element.AdminLevel_Child)&&el2.Element.Id_Parent!==globEl.Element.Id_Parent&&$(el2.Path).attr('d')!==''||parseInt(el2.Element.AdminLevel_Child)<parseInt(globEl.Element.AdminLevel_Child)&&el2.Element.Id_Parent===globEl.Element.Id_Parent).forEach((elem)=>{hidePath(elem.Path);});global.filter(el2=>el2.Element.Id_Parent===globEl.Element.Id_Parent&&parseInt(el2.Element.AdminLevel_Child)>=parseInt(globEl.Element.AdminLevel_Child)&&$(el2.Path).attr('d')===''&&(parseInt(globEl.Element.AdminLevel_Child)!==2)).forEach((elem)=>{showPath(elem.Path,elem.D);});if(childrenElements.length>0)
{let boundingRect=globEl.Path.getBBox();if(parseInt(globEl.Element.AdminLevel_Child)>1&&boundingRect.width<mapWidth/2&&boundingRect.height<mapHeight/2)
zoomOnElement(globEl,a.target.contentDocument);hidePath(globEl.Path);}
else
{hideLoader();span.innerHTML=globEl.Element.Name_Child;let matchingPins=loadedPins.filter(el=>el.Element_Id===clicked.at(-1).Element.Id);if(matchingPins.length>0)
{let fragment=document.createDocumentFragment();pinClicked=false;matchingPins=orderPins(matchingPins);let sel=document.createElement("select"),optionAux=document.createElement("option");if($('#container').children().length===0)
{optionAux.value=null;optionAux.text="Please choose an option:";sel.appendChild(optionAux);}
...
{let par=clicked.at(-2);showPath(par.Path,par.D);zoomOnElement(par,a.target.contentDocument);hidePath(par.Path);}
return;}
let elsLoaded=childrenElements.filter(el=>binarySearchGlobal(global,el.Id)!==-1).filter(el=>el.CountryCode_Child===globEl.Element.CountryCode_Child);if(elsLoaded.length>0&&elsLoaded[0].Id_Parent.split(', ').length<1)
{elsLoaded.forEach((el,index)=>{let pathToShow=binarySearchGlobal(global,el.Id);showPath(pathToShow.Path,pathToShow.D);});if(elsLoaded.length!==childrenElements.length&&elsLoaded.find(fNTC=>transContinental.find(tCont=>tCont!==fNTC.CountryCode_Child)!==undefined))
alert(`Number of elements loaded from memory is different than number of children! Loaded from memory: ${elsLoaded.length}, number of children(childrenElements.map(childEl => childEl.Id_Child )): ${childrenElements.length},
      ...
      number of children(childrenElements): ${childrenElements.length}`);hideLoader();span.innerHTML=globEl.Element.Name_Child;}
else if(txtLoaded.Id!==undefined&&globEl.Element.CountryCode_Child!==''&&globEl.Element.CountryCode_Child===txtLoaded.CC)
{let childrenLoaded=[];start=now();const indexes=childrenElements.map(el=>el.Index);function addToDocumentFragment(data,indexes)
{let fragment=document.createDocumentFragment(),jsonData=indexes.map(item=>jsonCountry[item]).filter(el=>binarySearch(loaded,el.Id)===-1),pathData=indexes.filter(el=>binarySearch(loaded,parseInt(data[el][1]))===-1).map(el=>data[el]);for(let i=0;i<jsonData.length;i++)
{const path=document.createElementNS("http://www.w3.org/2000/svg","path");path.setAttributeNS(null,"d",pathData[i][0]);path.setAttributeNS(null,"id",jsonData[i].Id_Child);path.setAttributeNS(null,"transform","scale(60, 60)");applyStyle(jsonData[i],path);fragment.appendChild(path);}
htmlPaths.append(fragment);end=now();printStats(`Finished creating Paths for ${globEl.Element.Name_Child}!`);}
addToDocumentFragment(txtLoaded.TxtData,indexes);start=now();for(let i=0;i<indexes.length;i++)
{let currValues=txtLoaded.TxtData[indexes[i]],[currPath,currId]=[currValues[0],parseInt(currValues[1])],foundChild=childrenElements[i],pathTemp=htmlPaths.children.namedItem(foundChild.Id_Child),globalElem=binarySearchGlobal(global,foundChild.Id);if(globalElem===-1)
{global.push({Element:foundChild,Path:pathTemp,D:$(pathTemp).attr('d'),Parents:[globEl],Children:[]});globalElem=global.at(-1);globalElem.Parents[0].Children.push(globalElem);}
else
showPath(globalElem.Path,globalElem.D);if(binarySearch(loaded,foundChild.Id)===-1)
{childrenLoaded.push(foundChild.Id_Child);loaded.push(foundChild.Id);pathsAsValues.push(pathTemp);addEvListener(pathTemp,global.length-1);};updateProgressBar(childrenLoaded.length,childrenElements.length);}
global=global.sort((a,b)=>a.Element.Id-b.Element.Id);loaded.sort((a,b)=>a-b);end=now();printStats(`Finished creating SVG paths for ${globEl.Element.Name_Child} from data in memory!`);hideLoader();span.innerHTML=globEl.Element.Name_Child;return;}
else
{let childrenImported=[];let fileToLoad=(parseInt(globEl.Element.AdminLevel_Child)<2)?`${txt1}${txt2}${txt3}`:`${txt1+countryCode}/${countryCode}${txt3}`;start=now();fetch(fileToLoad).then(response=>response.text()).then(text=>{function addToDocumentFragment(data,indexes)
{let fragment=document.createDocumentFragment(),jsonData=indexes.map(item=>((parseInt(globEl.Element.AdminLevel_Child)<2)?globalJson:jsonCountry)[item]).filter(el=>binarySearch(loaded,el.Id)===-1),country=+(parseInt(globEl.Element.AdminLevel_Child)<2),pathData=indexes.filter(el=>binarySearch(loaded,parseInt(data[el-country][1]))===-1).map(index=>data[index-country]);for(let i=0;i<jsonData.length;i++)
{const path=document.createElementNS("http://www.w3.org/2000/svg","path");path.setAttributeNS(null,"d",pathData[i][0]);path.setAttributeNS(null,"id",jsonData[i].Id_Child);path.setAttributeNS(null,"transform","scale(60, 60)");applyStyle(jsonData[i],path);fragment.appendChild(path);}
htmlPaths.append(fragment);end=now();printStats(`Finished creating Paths for ${globEl.Element.Name_Child}!`);}
end=now();printStats(`Finished loading txt for ${globEl.Element.Name_Child}!`);start=now();let parsedData=Papa.parse(text,{delimiter:':'});if(parseInt(globEl.Element.AdminLevel_Child)===2)
{[txtLoaded.CC,txtLoaded.Id,txtLoaded.Name,txtLoaded.TxtData]=[globEl.Element.CountryCode_Child,globEl.Element.Id,globEl.Element.Name_Child,parsedData.data];[jsonLoaded.CC,jsonLoaded.Id,jsonLoaded.JsonData,jsonLoaded.Name]=[txtLoaded.CC,txtLoaded.Id,jsonCountry,txtLoaded.Name];}
const indexes=childrenElements.map(el=>el.Index);addToDocumentFragment(parsedData.data,indexes);start=now();for(let i=0;i<indexes.length;i++)
{let foundChild=childrenElements[i],pathTemp=htmlPaths.children.namedItem(foundChild.Id_Child),globalElem=binarySearchGlobal(global,foundChild.Id);if(globalElem===-1)
{global.push({Element:foundChild,Path:pathTemp,D:$(pathTemp).attr('d'),Parents:[globEl],Children:[]});globalElem=global.at(-1);globalElem.Parents[0].Children.push(globalElem);}
else
showPath(globalElem.Path,globalElem.D);if(binarySearch(loaded,foundChild.Id)===-1)
{loaded.push(foundChild.Id);pathsAsValues.push(pathTemp);addEvListener(pathTemp,global.length-1);};childrenImported.push(foundChild.Id_Child);updateProgressBar(childrenImported.length,childrenElements.length);}
if(globEl.Element.Id===3)
{bringToTop(htmlPaths.children.namedItem('relation/20'));bringToTop(htmlPaths.children.namedItem('relation/10'));}
...
{let globEl=global.at(-1);globEl.Path.addEventListener('click',el=>{function svgPoint(element,x,y)
{var pt=$map.children()[0].createSVGPoint();pt.x=x;pt.y=y;return pt.matrixTransform(element.getScreenCTM().inverse());}
...
$('#reset').show();$('#span').show();let CC=findCountryCode(globEl);countryCode=(CC!=='')?CC:'undefined';if(parseInt(globEl.Element.AdminLevel_Child)<=1)
{childrenElements=globEl.Element.ChildrenIndexes.split(", ").map(el=>globalJson[el]);showLoader();applyEvList(globEl,globEl.Path);}
else
...
{jsonCountry.forEach((element,index)=>{let path=pathsAsValues.find((path)=>{return path.id===element.Id_Child;});if(path!==undefined&&global.find(el=>el.Element.Id_Child===path.id)===undefined)
...
showLoader();applyEvList(globEl,globEl.Path);return 0;}
else
{$.getJSON(`${txt1}${countryCode}/${countryCode}${txt4}`,function($jsonCountry)
{if(parseInt(globEl.Element.AdminLevel_Child)===2)
{global.filter(el=>parseInt(el.Element.AdminLevel_Child)<3&&el.Element.Id_Child!==globEl.Element.Id_Child&&$(el.Path).attr('d')!=='').forEach(elem=>hidePath(elem.Path))}
jsonCountry=$jsonCountry;if(binarySearch(loaded,globEl.Element.Id)===-1)
{jsonCountry.forEach((element,index)=>{let path=pathsAsValues.find((path)=>{return path.id===element.Id_Child;});if(path!==undefined&&global.find(el=>el.Element.Id_Child===path.id)===undefined)
{global.push({Element:element,Index:index,Path:path,D:(path!==undefined)?pathTargetJQ.attr('d'):undefined,Parents:undefined,Children:[]});global=global.sort((a,b)=>a.Element.Id-b.Element.Id);}});}
childrenElements=(globEl.Element.ChildrenIndexes!=="")?globEl.Element.ChildrenIndexes.split(", ").map(el=>jsonCountry[el]):"";if(childrenElements.length>0)
showLoader();applyEvList(globEl,globEl.Path);return 0;}).always(function()
...
path.classList.add('style-8');else if(parseInt(element.AdminLevel_Child)>=4)
{switch(countryCode)
{case'PT':diffCCode=false;switch(element.AdminLevel_Child)
{case'4':case'6':path.classList.add('style-4');break;case'7':path.classList.add('style-6');break;case'8':path.classList.add('style-7');break;case'10':path.classList.add('style-8');break;default:debugger;}
break;case'AD':case'GI':case'LI':case'MC':case'SM':diffCCode=false;path.classList.add('style-8');break;case'FR':if(element.CountryCode_Child==='GF'||element.CountryCode_Parent==='NC'||diffCCode)
{if(!diffCCode)
diffCCode=true;path.classList.add('style-5');}
else
{diffCCode=false;path.classList.add(`style-${element.AdminLevel_Child}`);}
break;default:diffCCode=false;path.classList.add(`style-${element.AdminLevel_Child}`);}}
else
{diffCCode=false;path.classList.add(`style-${element.AdminLevel_Child}`);}
return path;}
...
{zooming=true;$("#reset").prop("disabled",true);$("#back").prop("disabled",true);let boundingRect;if(e.classList!==undefined&&e.classList.length>0&&e.classList.contains('pin'))
{boundingRect=e.getBBox();$("#reset").prop("disabled",false);$("#back").prop("disabled",false);zooming=false;let viewBox=`${boundingRect.x*0.9999875} ${boundingRect.y*0.999975} ${boundingRect.width*3.33} ${boundingRect.height*3.33}`
svg.children[0].setAttribute(`viewBox`,viewBox);}
else
{boundingRect=e.Path.getBBox();if(parseInt(e.Element.AdminLevel_Child)>1)
{$("#reset").prop("disabled",false);$("#back").prop("disabled",false);zooming=false;let viewBox=`${boundingRect.x*resizeVar} ${boundingRect.y*resizeVar*0.99998375} ${boundingRect.width*1.08*resizeVar} ${boundingRect.height*1.08*resizeVar}`
svg.children[0].setAttribute(`viewBox`,viewBox);}
else
...
{$map[0].children[0].setAttribute(`viewBox`,`0 0 ${mapWidth} ${mapHeight}`);span.innerHTML=glob.Parents[0].Element.Name_Child;}
clicked.splice(clicked.indexOf(glob),1);}
else
{resetZoom($map[0]);}});$.getJSON(`${txt1}${txt7}${txt4}`,function($json)
{globalJson=$json;$('#back').hide();globalJson.filter(el=>emptyCountries.find(el2=>el2===el.CountryCode_Child)===undefined).forEach((element,index)=>{let path=pathsAsValues.find((path)=>{return path.id===element.Id_Child;});if(path!==undefined)
{global.push({Element:element,Path:path,D:path.attributes[0].value,Parents:undefined,Children:[]});applyStyle(element,path);if(parseInt(element.AdminLevel_Child)<0)
